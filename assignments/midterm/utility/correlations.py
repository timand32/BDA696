"""Adapted from cat_correlation.py, provided in class.
"""
...
def fill_na(data):
    if isinstance(data, pandas.Series):
        ...
        d = [value if value is not None else 0 for value in data]
...
def cat_cont_correlation_ratio(categories, values):
    ...
    y_total_avg = (
        numpy.sum(
            numpy.multiply(y_avg_array, n_array),
        )
        / numpy.sum(n_array)
    )
    numerator = numpy.sum(
    ...
    denominator = numpy.sum(
        numpy.power(
            numpy.subtract(values, y_total_avg),
            2,
        ),
    )
    if numerator == 0:
